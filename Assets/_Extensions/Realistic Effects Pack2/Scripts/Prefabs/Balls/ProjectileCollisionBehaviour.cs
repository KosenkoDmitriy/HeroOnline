...
public class ProjectileCollisionBehaviour : MonoBehaviour
{
    ...
    public LayerMask LayerMask = new LayerMask() { value = ~(1 << 8) };
    ...
    {
        if (effectSettings.Target == null) return;
        hit = new RaycastHit();
        onCollision = false;
        ...
        if (effectSettings.EffectRadius > 0.001f)
        {
    ...
    private void Update()
    {
        ...
        if (effectSettings.controller == null) return;
        string tagName = Helper.GetTagEnemy(effectSettings.controller);


        var direction = (endPoint - tRoot.position).normalized;
        RaycastHit raycastHit = new RaycastHit();

        float distanceToTarget = Vector3.Distance(effectSettings.controller.transform.position, endPoint);

        if (distanceToTarget < 1)
        {
            effectSettings.HandleCollider(effectSettings.Target);
            Destroy(gameObject);
        }
        else
        {
            if (Physics.Raycast(tRoot.position, direction, out raycastHit, distanceNextFrame + effectSettings.ColliderRadius, LayerMask))
            {
                ...
                if (raycastHit.collider.tag == tagName)
                {
                    ...
                    if (effectSettings.controller.isConnectedServer())
                    {
                        if (effectSettings.Target != null)
                        {
                            if (raycastHit.collider.name == effectSettings.Target.name)
                            {
        ...
        //  Debug.Log("CollisionEnter " + hit.collider.gameObject);
    ...
    private void CollisionEnter()
    {
        ...
        if (EffectOnHitObject != null && hit.transform != null)
        {
            ...
            MeshFilter meshFiler = hitGo.GetComponent<MeshFilter>();
            if (meshFiler != null)
            {
            ...
            //Debug.Log("raycastHit.collider " + hitGo);
